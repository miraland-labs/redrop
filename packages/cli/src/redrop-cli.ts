...
import {
  ...
  formatDropMessage,
  urlAndHandleFor,
} from './helpers/redrop/communication';
import {
  REDROP_TEMPORAL_SIGNER,
  REDROP_DISTRIBUTOR_ID,
  TOKEN_METADATA_PROGRAM_ID,
} from './helpers/constants';
import { getMetadata, loadRedropProgram } from './helpers/accounts';
...
programCommand('create')
  ...
  .option(
    '--delegate-only',
    'transfer and candy: delegate tokens from KEYPAIR instead of transferring to the redrop',
  )
  ...
  .option('--distribution-list <path>', 'List of users to build redrop from.')
  .option(
    '--resend-only',
  ...
  .option(
    '--host <string>',
    'Website to claim redrop',
    'https://redrop.miraland.top',
  )
  ...
  .action(async (options, cmd) => {
    ...
    switch (options.distributionMethod) {
      case 'wallets':
        temporalSigner = REDROP_DISTRIBUTOR_ID;
        break;
      case 'manual':
      case 'aws-email':
      case 'aws-sms':
    ...
    const instructions = await buildRedrop(
      connection,
      wallet.publicKey,
      options.distributionMethod,
      options.claimIntegration,
    ...
    if (typeof createResult === 'string') {
      ...
      console.log(
        'redrop creation succeeded',
        `https://explorer.miraland.top/tx/${createResult.txid}?cluster=${options.env}`,
      );
    }
...
programCommand('close')
  ...
  .option('--candy-machine <pubkey>', 'candy: public key of the candy machine')
  .option('--edition-mint <mint>', 'edition: mint of the master edition')
  .option('--base <path>', 'redrop authority generated on create')
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  .action(async (options, cmd) => {
    log.info(`Parsed options:`, options);
    ...
    const instructions = await closeRedrop(
      connection,
      wallet.publicKey,
      base,
      options.claimIntegration,
    ...
    if (typeof closeResult === 'string') {
      ...
      console.log(
        'redrop close succeeded',
        `https://explorer.miraland.top/tx/${closeResult.txid}?cluster=${options.env}`,
      );
    }
  });
...
programCommand('recover_update_authority')
  ...
  .action(async (options, cmd) => {
    ...
    const wallet = loadWalletKey(options.keypair);
    const base = loadWalletKey(options.base);
    const anchorProgram = await loadRedropProgram(wallet, options.env);
    ...
    if (typeof recoverResult === 'string') {
      ...
      console.log(
        'redrop recover succeeded',
        `https://explorer.miraland.top/tx/${recoverResult.txid}?cluster=${options.env}`,
      );
    }
  });
...
programCommand('fetch_program')
  .addHelpText(
    'before',
    'Utility to fetch the redrop program executable data. Useful for doing a diff against a local build. Bytes are formatted for easy comparison to `xxd` output',
  )
  .action(async options => {
    ...
    const executableBufferKey = new PublicKey(programPointer.data.subarray(4));
    ...
    const programData = programBuffer.data.subarray(0x30 - 3);
...
function programCommand(name: string) {
  return program
    ...
    .option(
      '-e, --env <string>',
      'Miraland cluster env name',
      'devnet-mln', //mainnet-mln, testnet-mln, devnet-mln
    )
    .option(
      '-k, --keypair <path>',
      `Miraland wallet location`,
