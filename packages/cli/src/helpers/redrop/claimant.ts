...
const explorerUrlFor = (env: string, key: string) => {
  return `https://explorer.miraland.top/address/${key}?cluster=${env}`;
};
...
export const validateEditionClaims = async (
  ...
  claimants.forEach((c, idx) => {
    ...
    if (c.amount !== 1) {
      throw new Error(
        `Claimant ${idx} has amount ${c.amount}. Expected 1 for edition redrop`,
      );
    }
  });
  ...
  // minted while a redrop is being created
...
export const buildRedrop = async (
  connection: RPCConnection,
  walletKey: PublicKey,
  commMethod: string,
  ...
  for (let idx = 0; idx < claimants.length; ++idx) {
    ...
    if (!needsPin) {
      ...
      const [claimantPda] = await PublicKey.findProgramAddress(
        // seeds.map(s => s.slice(0, 32)), // vanilla, deprecated in favor of subarray
        seeds.map(s => s.subarray(0, 32)),
        REDROP_DISTRIBUTOR_ID,
      );
    ...
    // the ability to use redrop for auction house winnings and such?
    const extra =
  ...
  for (let idx = 0; idx < claimants.length; ++idx) {
    ...
    if (!verified) {
      throw new Error('Redrop merkle tree verification failed');
    }
  ...
  instructions.push(
    new TransactionInstruction({
      programId: REDROP_DISTRIBUTOR_ID,
      keys: [
      ...
      data: Buffer.from([
        ...Buffer.from(sha256.digest('global:new_distributor')).subarray(0, 8),
...
export const closeRedrop = async (
  connection: RPCConnection,
  walletKey: PublicKey,
  base: Keypair,
  claimMethod: string,
