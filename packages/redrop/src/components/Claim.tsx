...
import { useAnchorWallet } from '@solarti/wallet-adapter-react';
import {
  AccountMeta,
...
export const chunk = (arr: Buffer, len: number): Array<Buffer> => {
  ...
  while (i < n) {
    ...
    chunks.push(arr.subarray(i, (i += len)));
  }
...
const buildMintClaim = async (
  ...
  if (!matches) {
    throw new Error('Redrop merkle proof does not match');
  }
...
const fetchNeedsTemporalSigner = async (
  ...
  if (!info.temporal.equals(REDROP_TEMPORAL_SIGNER)) {
...
export const Claim = (props: RouteComponentProps<ClaimProps>) => {
  ...
  React.useEffect(() => {
    ...
    const wrap = async () => {
      try {
        ...
        const [redropIdl, candyIdl] = await Promise.all([
          anchor.Program.fetchIdl(REDROP_DISTRIBUTOR_ID, provider),
          anchor.Program.fetchIdl(CANDY_MACHINE_ID, provider),
        ]);

        if (!redropIdl) throw new Error('Failed to fetch redrop IDL');
        if (!candyIdl) throw new Error('Failed to fetch candy machine IDL');

        setProgram({
          redrop: new anchor.Program(
            redropIdl,
            REDROP_DISTRIBUTOR_ID,
            provider,
          ),
  ...
  const sendOTP = async (e: React.SyntheticEvent) => {
    ...
    if (instructions.setup !== null && instructions.setup.length !== 0) {
      ...
      // mira
      setupTx.recentBlockhash = latestBlockhash.blockhash;
    ...
    const txnNeedsTemporalSigner = claimTx.signatures.some(s =>
      s.publicKey.equals(REDROP_TEMPORAL_SIGNER),
    )
      ? claimTx
      : setupTx &&
  ...
  const verifyOTP = async (
    ...
    if (txnNeedsTemporalSigner && !skipAWSWorkflow) {
      ...
      txnNeedsTemporalSigner.addSignature(REDROP_TEMPORAL_SIGNER, sig);
    }
  ...
  const populateClaimC = onClick => (
    <React.Fragment>
      ...
      <Box sx={{ position: 'relative' }}>
        ...
        >
          {asyncNeedsTemporalSigner ? 'Next' : 'Claim Redrop'}
        </Button>
        {loading && loadingProgress()}
      </Box>
    </React.Fragment>
